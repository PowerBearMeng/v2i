...
class RosToZmqBridgeNode(Node):
    ...
    def __init__(self):
        ...
        # --- 1. 设置 ZMQ 发布者
        self.get_logger().info("正在初始化 ZeroMQ Publisher...")
        self.zmq_context = zmq.Context()
        self.zmq_publisher = self.zmq_context.socket(zmq.PUB)
        self.zmq_publisher.bind("tcp://*:5555")
        self.get_logger().info("ZMQ Publisher 已绑定到 tcp://*:5555")
    ...
    def lidar_callback(self, msg: PointCloud2):
        # 探针 0: 确认回调函数是否被触发
        self.get_logger().debug("---------- New Callback Triggered ----------")
        ...
        try:
            # 探针 1: 确认锁已获取
            self.get_logger().debug("[DEBUG-STEP 1] Lock acquired.")

            # proc_start_time = time.time()
            pcd_current_raw = pointcloud2_to_open3d(msg, self.my_roi)
            
            # 探针 2: 确认点云转换完成
            self.get_logger().debug(f"[DEBUG-STEP 2] PointCloud converted. Point count: {len(pcd_current_raw.points)}")
            ...
            pcd_current_clean = pcd_current_clean
            # 探针 3: 确认预处理完成
            
            if self.pcd_prev_clean is None:
                self.pcd_prev_clean = pcd_current_clean
                self.frame_index += 1
                self.get_logger().info(f"已处理第一帧 (ID: {self.frame_index})，作为后续比较的基准。")
                self.processing_lock.release()
                return
            proc_start_time = time.time()
            ...
            # 探针 4: 确认动态检测完成
            self.get_logger().debug(f"[DEBUG-STEP 4] Detection finished. Dynamic point count: {len(pcd_current_clean.points)}")

            # proc_end_time = time.time()

            self.frame_index += 1

            if pcd_current_clean.has_points():
                dyn_points_np = np.asarray(pcd_current_clean.points, dtype=np.float32)

                message_to_send = {
                    'frame_id': self.frame_index,
                    'proc_time_ms': (proc_end_time - proc_start_time) * 1000,
                    'dynamic_points': dyn_points_np,
                    'static_points_count': len(pcd_current_clean.points)
                }

                packed_message = msgpack.packb(message_to_send, default=m.encode)
                self.zmq_publisher.send(packed_message)
                self.get_logger().info(f"已处理第 {self.frame_index} 帧, 耗时 {(proc_end_time - proc_start_time) * 1000:.2f} ms, 发送 {len(dyn_points_np)} 个动态点。")
            else:
                 self.get_logger().info(f"已处理第 {self.frame_index} 帧, 耗时 {(proc_end_time - proc_start_time) * 1000:.2f} ms, 未检测到动态点。")

            # 探针 5: 确认本次回调即将正常结束
            self.get_logger().debug("[DEBUG-STEP 5] Callback ending normally.")

        except Exception as e:
            # 关键的异常捕捉！如果程序有任何错误，都会在这里打印出来
            self.get_logger().fatal(f"!!!!!! An unexpected error occurred in lidar_callback !!!!!!")
            self.get_logger().fatal(f"Error Type: {type(e).__name__}")
            self.get_logger().fatal(f"Error Message: {e}")
            import traceback
            ...
            if self.processing_lock.locked():
                self.get_logger().debug("[DEBUG-STEP 6] Releasing lock in finally block.")
                self.processing_lock.release()
