...
from utils.config import PREPROCESSING_CONFIG, M_DETECTION, MODEL_PATH
...
class RosToZmqBridgeNode(Node):
    ...
    def __init__(self):
        ...
        self.detector = MDetector(config=M_DETECTION)
    ...
    def lidar_callback(self, msg: PointCloud2):
        ...
        if not self.processing_lock.acquire(blocking=False):
            self.get_logger().info("处理速度跟不上，丢弃一帧数据以保持实时性。")
            return
        ...
        try:
            ...
            proc_start_time = time.time()
            static_pcd, dynamic_pcd = self.detector.process_frame(pcd_current_clean)
